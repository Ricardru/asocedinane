...
interface CategoriaIngresoFormProps {
  categoria?: CategoriaIngreso
  onClose: () => void
}

export function CategoriaIngresoForm({ categoria, onClose }: CategoriaIngresoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      alert(`Error al ${categoria ? 'actualizar' : 'crear'} categoría de ingreso: ${error.message}`)
    } else {
      onClose()
      ...
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
        <div className="mt-3">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">
            {categoria ? 'Editar' : 'Nueva'} Categoría de Ingreso
          </h3>
