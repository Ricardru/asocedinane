...
import { CategoriaIngresoForm } from './CategoriaIngresoForm'
...
interface CategoriaIngresoListProps {
  categorias: CategoriaIngreso[]
}

export function CategoriaIngresoList({ categorias }: CategoriaIngresoListProps) {
  const [editingCategoria, setEditingCategoria] = useState<CategoriaIngreso | null>(null)
  ...
  const handleToggleActive = async (id: string, activo: boolean) => {
    ...
    if (error) {
      alert('Error al actualizar categoría de ingreso: ' + error.message)
    } else {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Categorias_Ingresos')
    XLSX.writeFile(wb, `categorias_ingresos_${new Date().toISOString().split('T')[0]}.xlsx`)
  }
  ...
  const exportToPDF = () => {
    ...
    // Título
    doc.setFontSize(18)
    doc.text('Listado de Categorías de Ingresos', 14, 22)
    ...
    doc.save(`categorias_ingresos_${new Date().toISOString().split('T')[0]}.pdf`)
  }
  ...
  const printList = () => {
    ...
    const htmlContent = `
      <html>
        <head>
          <title>Listado de Categorías de Ingresos</title>
          <style>
        ...
        <body>
          <div class="header">
            <h1>Listado de Categorías de Ingresos</h1>
          </div>
          <div class="date">
    ...
    <>
      ...
      {viewMode === 'list' ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {/* Tarjeta para agregar nueva categoría de ingreso */}
          <div
            ...
            title="Agregar nueva categoría de ingreso"
          >
            <div className="text-center">
              ...
              <span className="text-lg font-semibold text-green-600 dark:text-green-400">Agregar Categoría de Ingreso</span>
            </div>
          </div>

          {filteredCategorias.map((categoria) => (
            <div key={categoria.id} className="relative bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 pb-12 border border-gray-200 dark:border-gray-700">
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Nueva Categoría de Ingreso</h3>
